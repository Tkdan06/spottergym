...
export function WorkoutsCoachPage() {
  ...
  return (
    <main className="page workouts-page workouts-coach-page">
      <div className="subpage-top">
        ...
        <header className="page-header">
          <div className="page-header-text workouts-head">
            <h1 className="page-title">Разбор недели</h1>
      ...
      {!loading && coach && !letter && coach.canGenerate ? (
        <section className="surface workouts-empty">
          ...
          <p className="muted">
            GigaChat посмотрит дневник и напишет, засчиталась ли неделя и что делать в следующий
            заход.
          </p>
      ...
      {!loading && coach && !letter && !coach.canGenerate ? (
        <section className="surface workouts-empty">
          ...
          <p className="empty-copy-title">
            {coach.status === 'locked' ? 'Пока рано' : 'Разбор появится позже'}
          </p>
          <p className="muted">
            {coach.status === 'locked'
              ? needCopy(coach.sessionsNeeded)
              : 'Разбор сейчас недоступен. Цифры из дневника — ниже.'}
          </p>
          <FactChips coach={coach} />
          <Link to="/app/workouts/new" className="btn btn-primary btn-block">
            <Plus size={16} /> Записать тренировку
      ...
      {!loading && !coach && !error ? (
        <section className="surface workouts-empty">
          ...
          <p className="muted">Не удалось получить статус. Попробуй ещё раз с экрана тренировок.</p>
        </section>
      ) : null}
    </main>
  )
