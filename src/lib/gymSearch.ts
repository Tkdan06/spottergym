...
/** Пары для lat→cyr: длинные диграфы первыми */
...
/** Известные написания сетей/клубов (оба направления) */
...
/** Нормализация для сравнения: пунктуация → пробел, кириллица → латиница */
...
export function gymSearchBlob(gym: {
  name: string
  network: string
  district?: string
  address?: string
}): string {
  return `${gym.name} ${gym.network} ${gym.district || ''} ${gym.address || ''}`.toLowerCase()
}
...
export function gymMatchesQuery(
  gym: { name: string; network: string; district?: string; address?: string },
  qRaw: string,
): boolean {
  const q = qRaw.toLowerCase().trim()
  if (!q) return true
  const hay = gymSearchBlob(gym)
